...
//	for (auto const &f : p_pFahrzeuge) { //funktioniert nicht ordnungsgemäß
//		try{
//			f.get()->vSimulieren();//namen aller fahrzeuge zusammen in einen string
//			f.get()->vZeichnen(*this);
...
//	p_pFahrzeuge.vAktualisieren();
		...
		for (vIterator iter = p_pFahrzeuge.begin(); iter != p_pFahrzeuge.end();){ //ausweich lösung!!
			try{
				(*iter)->vSimulieren();
				(*iter)->vZeichnen(*this);
				iter++;
