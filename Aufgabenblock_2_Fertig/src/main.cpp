...
int vAufgabe_6(){
	...
	bInitialisiereGrafik(800, 500);
		...
		cout << innerOrts.getName()<<endl;
		bZeichneStrasse(innerOrts.getName(), ausserOrts.getName(), 500, 2, koordinaten);
...
int vAufgabe_7(){
	...
	kr1->vAnnahme(move(f1),3);	//übergabe der Fahrzeuge in die Fahrzeugliste. Wichtig besitzübergabe mit move()!
	kr1->vAnnahme(move(f2));
	kr1->vAnnahme(move(f3));
	kr1->vAnnahme(move(Fahrad));

	Weg::vKopf();
	kr1->coutWege();			//sollte alle wege mit deren parametern ausgeben (nicht fertig!)
	kr2->coutWege();
	kr3->coutWege();
	kr4->coutWege();
...
int main() { // Globale Zeit festlegen. Start bei 00:00 (0)
	...
	//vAufgabe_4();
	//vAufgabe_5();
	//vAufgabe_6();			//server stört die namen von den wegen! Vermutung wege als normale objekte erstellt...
	//vAufgabe_6_2();
	//vAufgabe_6a();
	vAufgabe_7();
